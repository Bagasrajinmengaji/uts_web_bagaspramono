...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['file_transaksi'])) {
    ...
    if (!in_array($extension, ['csv', 'txt'], true)) {
        set_flash_message('danger', 'Format berkas tidak didukung. Silakan unggah file CSV yang disimpan dari Excel.');
        header('Location: dashboard.php');
        exit;
    }
    ...
    if (!$header || count($header) < 4) {
        fclose($handle);
        set_flash_message('danger', 'Format header kolom salah. Pastikan berkas memiliki minimal 4 kolom: Tanggal, Jenis, Nominal, Keterangan.');
        header('Location: dashboard.php');
        exit;
    }
    ...
    try {
        ...
        while (($row = fgetcsv($handle, 1000, $delimiter)) !== false) {
            ...
            if (!$tanggal) {
                throw new Exception("Baris ke-$row_num: Format Tanggal '$raw_tanggal' tidak valid. Gunakan format YYYY-MM-DD atau DD/MM/YYYY.");
            }
            ...
            if (!in_array($jenis, ['Pemasukan', 'Pengeluaran'], true)) {
                throw new Exception("Baris ke-$row_num: Jenis transaksi '$raw_jenis' tidak valid. Harus berupa 'Pemasukan' atau 'Pengeluaran'.");
            }
            
            // 3. Validasi & Format Nominal
            $nominal = clean_import_nominal($raw_nominal);
            if ($nominal === false || $nominal <= 0) {
                throw new Exception("Baris ke-$row_num: Nominal '$raw_nominal' tidak valid. Harus berupa angka positif.");
            }
            
            // 4. Validasi Keterangan
            $keterangan = trim($raw_keterangan);
            if (empty($keterangan)) {
                throw new Exception("Baris ke-$row_num: Keterangan tidak boleh kosong.");
            }
        ...
        if ($imported_count === 0) {
            $pdo->rollBack();
            set_flash_message('danger', 'Tidak ada transaksi baru yang diimpor. Berkas kosong.');
        } else {
            // Commit semua perubahan jika sukses semua
            $pdo->commit();
            set_flash_message('success', "Berhasil mengimpor $imported_count transaksi baru!");
        }
