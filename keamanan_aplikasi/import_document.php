...
function unzip_docx_data($zip_content, $target_file)
{
    ...
    while (($pos = strpos($zip_content, "\x50\x4b\x03\x04", $pos)) !== false) {
        ...
        $data = unpack(
            "vversion/vflag/vmethod/vtime/vdate/Vcrc/Vcsize/Vucsize/vfnlen/viflen",
            substr($header, 4),
        );
...
function parse_docx_xml_table($xml_content)
{
    ...
    foreach ($rows as $row) {
        ...
        if (count($cells) < 4) {
            continue;
        } // Lewati jika kolom tidak lengkap
...
function parse_html_table($html_content)
{
    ...
    foreach ($rows as $row) {
        ...
        if ($tds->length < 4) {
            continue;
        } // Lewati jika data tidak lengkap atau baris total
    ...
    try {
        if (in_array($extension, ["docx", "doc"], true)) {
            ...
            if (substr($file_content, 0, 4) === "\x50\x4b\x03\x04") {
                ...
                $xml_content = unzip_docx_data(
                    $file_content,
                    "word/document.xml",
                );
                if (!$xml_content) {
                    throw new Exception(
                        "Gagal mengekstrak data dari berkas Word DOCX.",
                    );
                }
            ...
            if ($nominal_index === false) {
                throw new Exception(
                    "Format berkas PDF salah atau tidak dibuat oleh DompetKu.",
                );
            }
            ...
            foreach ($slice as $text) {
                ...
                if (
                    strpos($text_lower, "total") === 0 ||
                    strpos($text_lower, "catatan") === 0 ||
                    empty($text)
                ) {
                    break;
                }
            ...
            if ($xlsx = \Shuchkin\SimpleXLSX::parse($file["tmp_name"])) {
                ...
                if (empty($rows)) {
                    throw new Exception(
                        "Berkas Excel kosong atau tidak dapat dibaca.",
                    );
                }
                ...
                if (count($header) < 4) {
                    throw new Exception(
                        "Format header kolom Excel salah. Harus memiliki minimal 4 kolom: Tanggal, Jenis, Nominal, Keterangan.",
                    );
                }
                ...
                foreach ($header as $idx => $col_name) {
                    ...
                    if (strpos($clean_name, "tanggal") !== false) {
                        ...
                        strpos($clean_name, "nominal") !== false ||
                        strpos($clean_name, "jumlah") !== false
                    ) {
                ...
                if ($col_indices["tanggal"] === -1) {
                    $col_indices["tanggal"] = 0;
                }
                if ($col_indices["jenis"] === -1) {
                    $col_indices["jenis"] = 1;
                }
                ...
                if ($col_indices["kategori"] === -1) {
                    if (count($header) >= 5) {
                        ...
                        if ($col_indices["nominal"] === -1) {
                            $col_indices["nominal"] = 3;
                        }
                        if ($col_indices["keterangan"] === -1) {
                            $col_indices["keterangan"] = 4;
                        }
                    } else {
                        $col_indices["kategori"] = -1;
                        if ($col_indices["nominal"] === -1) {
                            $col_indices["nominal"] = 2;
                        }
                        if ($col_indices["keterangan"] === -1) {
                            $col_indices["keterangan"] = 3;
                        }
                    }
                } else {
                    if ($col_indices["nominal"] === -1) {
                        $col_indices["nominal"] =
                            $col_indices["kategori"] === 2 ? 3 : 2;
                    }
                    if ($col_indices["keterangan"] === -1) {
                        $col_indices["keterangan"] =
                            $col_indices["kategori"] === 2 ? 4 : 3;
                    }
                }
                ...
                for ($i = 1; $i < count($rows); $i++) {
                    ...
                    $val_tanggal = isset($row[$col_indices["tanggal"]])
                        ? $row[$col_indices["tanggal"]]
                        : "";
                    $val_jenis = isset($row[$col_indices["jenis"]])
                        ? $row[$col_indices["jenis"]]
                        : "";
                    $val_kategori =
                        $col_indices["kategori"] !== -1 &&
                        isset($row[$col_indices["kategori"]])
                            ? $row[$col_indices["kategori"]]
                            : "";
                    $val_nominal = isset($row[$col_indices["nominal"]])
                        ? $row[$col_indices["nominal"]]
                        : "";
                    $val_keterangan = isset($row[$col_indices["keterangan"]])
                        ? $row[$col_indices["keterangan"]]
                        : "";
        ...
        if (empty($rows_data)) {
            throw new Exception(
                "Tidak ada baris data transaksi yang berhasil ditemukan di dalam dokumen.",
            );
        }
        ...
        foreach ($rows_data as $row) {
            ...
            if (!$tanggal) {
                throw new Exception(
                    "Format Tanggal '$raw_tanggal' tidak valid. Gunakan format YYYY-MM-DD atau DD/MM/YYYY.",
                );
            }
            ...
            if (!in_array($jenis, ["Pemasukan", "Pengeluaran"], true)) {
                throw new Exception(
                    "Jenis transaksi '$raw_jenis' tidak valid. Harus berupa 'Pemasukan' atau 'Pengeluaran'.",
                );
            }
            ...
            if ($nominal === false || $nominal <= 0) {
                throw new Exception(
                    "Nominal '$raw_nominal' tidak valid. Harus berupa angka positif.",
                );
            }
        ...
        if ($imported_count === 0) {
            ...
            set_flash_message(
                "danger",
                "Tidak ada transaksi baru yang diimpor.",
            );
        } else {
            $pdo->commit();
            set_flash_message(
                "success",
                "Berhasil mengimpor $imported_count transaksi baru dari dokumen!",
            );
        }
    } catch (Exception $e) {
        ...
        set_flash_message(
            "danger",
            "Gagal mengimpor berkas! " . $e->getMessage(),
        );
    }
