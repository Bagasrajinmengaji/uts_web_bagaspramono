...
try {
    if ($id !== '') {
        $query = "SELECT t.*, k.nama_kategori FROM transaksi t LEFT JOIN kategori k ON t.id_kategori = k.id_kategori WHERE t.id = :id AND t.user_id = :user_id";
...
$sheet->getStyle('A1')->getFont()->setName('Segoe UI')->setSize(16)->setBold(true);
$sheet->getStyle('A1')->getAlignment()->setHorizontal(Alignment::HORIZONTAL_LEFT);

$sheet->setCellValue('A2', 'Aplikasi Pencatatan Keuangan Pribadi yang Aman & Presisi');
$sheet->mergeCells('A2:F2');
$sheet->getStyle('A2')->getFont()->setName('Segoe UI')->setSize(10)->setItalic(true)->setColor(new \PhpOffice\PhpSpreadsheet\Style\Color('555555'));
...
if ($jenis_filter !== '') {
    ...
    $sheet->getStyle('A' . $metadata_end_row)->getFont()->setBold(true);
}
if ($search !== '') {
    $metadata_end_row++;
...
if (empty($transactions)) {
    ...
    $sheet->getStyle('A' . $current_row . ':F' . $current_row)->getBorders()->getAllBorders()->setBorderStyle(Border::BORDER_THIN);
    $current_row++;
} else {
    $no = 1;
    foreach ($transactions as $row) {
        $date_formatted = date('Y-m-d', strtotime($row['tanggal']));
        $kategori_name = $row['nama_kategori'] ? $row['nama_kategori'] : 'Tanpa Kategori';
        ...
        $sheet->setCellValue('D' . $current_row, $row['keterangan']);
        $sheet->setCellValue('E' . $current_row, (float)$row['nominal']);
        $sheet->setCellValue('F' . $current_row, $row['jenis']);
        
        // Zebra Striping (Baris genap abu-abu sangat tipis #F9F9F9)
        if ($no % 2 === 0) {
            $sheet->getStyle('A' . $current_row . ':F' . $current_row)->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setRGB('F9F9F9');
        }
        
        // Alignment per kolom
        $sheet->getStyle('A' . $current_row)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER); // No
        $sheet->getStyle('B' . $current_row)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER); // Tanggal
        $sheet->getStyle('C' . $current_row)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER); // Kategori
        $sheet->getStyle('D' . $current_row)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_LEFT);   // Keterangan
        $sheet->getStyle('E' . $current_row)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT);  // Nominal
        $sheet->getStyle('F' . $current_row)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER); // Tipe
        
        // Format Mata Uang Rupiah untuk Nominal
        $sheet->getStyle('E' . $current_row)->getNumberFormat()->setFormatCode('_-* "Rp" #,##0_-;\-* "Rp" #,##0_-;_-* "-"_-;_-@_-');
        
        // Border tipis untuk sel data
        $sheet->getStyle('A' . $current_row . ':F' . $current_row)->getBorders()->getAllBorders()->setBorderStyle(Border::BORDER_THIN)->getColor()->setRGB('D3D3D3');
    ...
    $sheet->mergeCells('A' . $current_row . ':D' . $current_row);
    $sheet->setCellValue('E' . $current_row, '=SUMIF(F' . $start_data_row . ':F' . $last_data_row . ', "Pemasukan", E' . $start_data_row . ':E' . $last_data_row . ')');
    
    // Styling Total Pemasukan (Abu-abu Muda, Bold, Border Atas-Bawah)
    $pemasukan_range = 'A' . $current_row . ':F' . $current_row;
    $sheet->getStyle($pemasukan_range)->applyFromArray([
        ...
        'borders' => [
            'top' => ['borderStyle' => Border::BORDER_THIN, 'color' => ['rgb' => 'A0A0A0']],
            'bottom' => ['borderStyle' => Border::BORDER_THIN, 'color' => ['rgb' => 'A0A0A0']]
        ]
    ]);
    $sheet->getStyle('A' . $current_row)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT);
    $sheet->getStyle('E' . $current_row)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_RIGHT);
    $sheet->getStyle('E' . $current_row)->getNumberFormat()->setFormatCode('_-* "Rp" #,##0_-;\-* "Rp" #,##0_-;_-* "-"_-;_-@_-');
    ...
    $sheet->mergeCells('A' . $current_row . ':D' . $current_row);
    $sheet->setCellValue('E' . $current_row, '=SUMIF(F' . $start_data_row . ':F' . $last_data_row . ', "Pengeluaran", E' . $start_data_row . ':E' . $last_data_row . ')');
    
    // Styling Total Pengeluaran (Abu-abu Muda, Bold, Border Atas-Bawah)
    $pengeluaran_range = 'A' . $current_row . ':F' . $current_row;
    ...
    $sheet->mergeCells('A' . $current_row . ':D' . $current_row);
    $sheet->setCellValue('E' . $current_row, '=E' . $row_pemasukan . '-E' . $row_pengeluaran);
    
    // Styling Saldo Akhir (Biru Muda Lembut #D9E1F2, Bold, Border Atas-Bawah Akuntansi Double)
    $saldo_range = 'A' . $current_row . ':F' . $current_row;
    $sheet->getStyle($saldo_range)->applyFromArray([
        'font' => ['bold' => true, 'name' => 'Segoe UI', 'color' => ['rgb' => '000000']],
        'fill' => [
            'fillType' => Fill::FILL_SOLID,
